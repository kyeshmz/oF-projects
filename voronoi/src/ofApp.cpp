...
void ofApp::update(){
    ...
    if (recording) {
        ...
        if (cursize >= endpoints) {
            cout << "done! drew " << voronoi.getCells().size() << " cells" << endl;
            recording = false;
            needsRedraw = false;
            savelast = true;
        } else  {
    ...
    for (int i = 0; i < voronoi.getCells().size(); i++) {
        ofxVoronoiCell cell = voronoi.getCells().at(i);
...
//
//void
//ofApp::guiEvent(ofxUIEventArgs &e)
//{
